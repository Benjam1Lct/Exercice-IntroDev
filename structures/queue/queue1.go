...
/*
On considère la structure de file d'attente (queue) définie ci-dessous. Une queue contient un certain nombre de places
(sa taille) et est définie par la dernière place dans cette queue.
...
# Entrées
- q : une queue déjà formée, cette queue sera modifiée par la méthode pour y ajouter une place contenant la personne
qui souhaite s'ajouter à la queue
...
func (q *queue) ajout(p personne) {
	...
	if q.taille == 0 {
		...
		placePrecedente := q.fin
		nouvellePlace.precedant = placePrecedente
		q.fin = nouvellePlace
	}
